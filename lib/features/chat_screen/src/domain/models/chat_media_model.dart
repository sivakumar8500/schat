  ...
  factory ChatMediaModel.fromJson(Map<String, dynamic> json) {
    ...
    if (content is Map) {
      ...
      return ChatMediaModel(
        ...
        fileSizeBytes: int.tryParse((contentMap['fileSize'] ?? contentMap['file_size_bytes'])?.toString() ?? '') ?? 0,
        status: 'completed',
    ...
    return ChatMediaModel(
      ...
      fileSizeBytes: int.tryParse((json['file_size_bytes'] ?? json['fileSizeBytes'])?.toString() ?? '') ?? 0,
