  ...
  Future<void> _checkPermissionsAndFetchLocation() async {
    ...
    try {
      ...
      if (_selectedPosition == null) {
        if (mounted) context.showErrorNotification('Failed to get location: $e');
        setState(() => _permissionDenied = true);
      }
    }
  }
    ...
    _debounce = Timer(const Duration(milliseconds: 500), () async {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (data['suggestions'] != null) {
            setState(() {
              _searchResults = List<Map<String, dynamic>>.from(
                data['suggestions'].map((s) => s['placePrediction']).where((p) => p != null)
              );
            });
          } else {
          ...
          final data = json.decode(response.body);
          debugPrint('Places API (New) Error: ${response.statusCode} - ${data['error']?['message']}');
          setState(() => _searchResults = []);
        }
      } catch (e) {
  ...
  Future<void> _selectPlace(String placeId, String description) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('Places Details API Error: ${response.statusCode} - ${response.body}');
      }
    } catch (e) {
            ...
            ConstrainedBox(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final mainText = place['structuredFormat']?['mainText']?['text'] ?? place['text']?['text'] ?? 'Unknown place';
                  final secondaryText = place['structuredFormat']?['secondaryText']?['text'] ?? '';
                  
                  return ListTile(
                    leading: const Icon(Icons.location_on, color: Colors.grey),
                    title: Text(mainText),
                    subtitle: secondaryText.isNotEmpty ? Text(secondaryText) : null,
                    onTap: () => _selectPlace(place['placeId'], mainText),
                  );
                },
              ),
            )
